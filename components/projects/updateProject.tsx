...
import { updateProjectClient } from '@/actions/projects'
import { IconEdit, IconRevert, IconWarning } from '@/app/lib/icons'
...
function EditBtn({ field, active, onUnlock, onLock, onCancel, changed, onUndo, ariaLabel }: EditBtnProps) {
...
export default function UpdateProjectForm({
  ...
  async function save(): Promise<boolean> {
    ...
    try {
      await toast.promise(
        updateProjectClient(id, credentials, {
    ...
    <>
      ...
      <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 py-10 pb-24">
        <form id="update-project-form" onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* ── Locked banner ──────────────────────────────────── */}
          {readonly && (
            <div className="flex items-center gap-2 px-4 py-3 rounded-lg bg-warning-bg text-warning text-sm border border-warning/30">
              <IconWarning className="shrink-0 text-xs" />
              Prosjektet er ikke lenger redigerbart.
            </div>
          )}

          {/* ── Form sections (single elevated surface) ─────────── */}
          <div className="card-subtle p-0 overflow-hidden">
            ...
            <section className="p-6 space-y-6">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {!readonly && (
                    <EditBtn field="educationField" {...editProps}
                      ...
                      ariaLabel="Rediger linje" />
                  )}
                </div>
                {!readonly && activeField === 'educationField' ? (
                  <LinjeDropdown
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <label htmlFor="proj-title" className="label">Prosjekttittel</label>
                  {!readonly && (
                    <EditBtn field="title" {...editProps}
                      changed={isFieldChanged('title')} onUndo={() => undoField('title')}
                      ariaLabel="Rediger prosjekttittel" />
                  )}
                </div>
                {!readonly && activeField === 'title' ? (
                  <input id="proj-title" type="text" className="input animate-fade-in" value={values.title} onChange={(e) => set('title')(e.target.value)} />
                ) : (
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <label htmlFor="proj-description" className="label">Beskrivelse</label>
                  {!readonly && (
                    <EditBtn field="description" {...editProps}
                      changed={isFieldChanged('description')} onUndo={() => undoField('description')}
                      ariaLabel="Rediger beskrivelse" />
                  )}
                </div>
                {!readonly && activeField === 'description' ? (
                  <textarea id="proj-description" className="input min-h-30 animate-fade-in" value={values.description} onChange={(e) => set('description')(e.target.value)} />
                ) : (
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {!readonly && (
                    <EditBtn field="budget" {...editProps}
                      ...
                      ariaLabel="Rediger budsjettramme" />
                  )}
                </div>
                {!readonly && activeField === 'budget' ? (
            ...
            <section className="p-6">
              ...
              <div className="divide-y divide-border">
                ...
                <div className="py-3 space-y-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    <label htmlFor="proj-forename" className="label">Navn</label>
                    {!readonly && (
                      <EditBtn field="name" {...editProps}
                        changed={isFieldChanged('name')} onUndo={() => undoField('name')}
                        ariaLabel="Rediger navn" />
                    )}
                  </div>
                  {!readonly && activeField === 'name' ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 animate-fade-in">
                      <input id="proj-forename" type="text" className="input" placeholder="Fornavn"   value={values.clientForename} onChange={(e) => set('clientForename')(e.target.value)} />
                      <input id="proj-surname"  type="text" className="input" placeholder="Etternavn" value={values.clientSurname}  onChange={(e) => set('clientSurname')(e.target.value)} />
                    </div>
                  ) : (
                ...
                <div className="py-3 space-y-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    <label htmlFor="proj-email" className="label">E-post</label>
                    {!readonly && (
                      <EditBtn field="email" {...editProps}
                        changed={isFieldChanged('email')} onUndo={() => undoField('email')}
                        ariaLabel="Rediger e-postadresse" />
                    )}
                  </div>
                  {!readonly && activeField === 'email' ? (
                    <input id="proj-email" type="email" className="input animate-fade-in" value={values.clientEmail} onChange={(e) => set('clientEmail')(e.target.value)} />
                  ) : (
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {!readonly && (
                      <EditBtn field="phone" {...editProps}
                        ...
                        ariaLabel="Rediger telefonnummer" />
                    )}
                  </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {!readonly && (
                        <EditBtn field="org" {...editProps}
                          ...
                          ariaLabel="Rediger organisasjon" />
                      )}
                    </div>
                    {!readonly && activeField === 'org' ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 animate-fade-in">
                        <div className="space-y-1">
                          <label htmlFor="proj-org-name" className="label">Navn</label>
                          <input id="proj-org-name" type="text" className="input" placeholder="Firma AS" value={values.organizationName} onChange={(e) => set('organizationName')(e.target.value)} />
                        </div>
                        <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {!readonly && (
                      <EditBtn field="address" {...editProps}
                        ...
                        ariaLabel="Rediger adresse" />
                    )}
                  </div>
                  {!readonly && activeField === 'address' ? (
                    <div className="animate-fade-in">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {!readonly && (
                        <EditBtn
                          ...
                          ariaLabel="Rediger fakturaadresse"
                        />
                      )}
                    </div>
                    {!readonly && activeField === 'billingAddress' && (
        ...
        <div className="fixed bottom-0 left-0 right-0 z-10 bg-surface border-t border-border shadow-t-md">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="grid items-center">
              <p className={`row-start-1 col-start-1 small-text text-warning transition-all duration-200 ${
