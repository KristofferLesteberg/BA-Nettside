  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes}>
      <div className={`transition duration-200 ease-in
        ${isLeaving ? "opacity-0 scale-75" : isDragging ? "" : "animate-fade-in"}`}>
        <div
          onClick={isDragging ? undefined : onImageClick}
          className={`w-50 h-27.5 flex items-center justify-center overflow-hidden
            rounded-md bg-surface cursor-pointer border-2 border-secondary
            transition-shadow duration-200
            ${isDragging ? "shadow-xl shadow-secondary/50" : "shadow-md shadow-secondary/35"}`}
        >
          <img src={src} className="max-w-full max-h-full object-contain" alt="" />
        </div>
        <div className={`grid grid-cols-3 items-center px-1 py-0.5 mt-3
          rounded-md border border-border bg-surface transition-shadow duration-200
